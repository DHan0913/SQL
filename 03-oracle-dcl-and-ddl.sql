...
-- Oracle 18버전부터 Contatiner Database 개념 도입 
-- 계정 생성 방법1. 사용자 계정 C##
...
-- 아직 접속 불가
-- 데이터베이스 접속, 테이블 생성 데이터베이스 객체 작업을 수행
...
SELECT
    *
FROM
    hr.departments;   --hr.departments에 대한 권한은 없다.
...
-- 나중에 author_id 컬럼 추가 -> author.author_id와 참조 연결 예정
ALTER TABLE book DROP COLUMN author;
DESC book;

-- book 테이블에 author_id 컬럼 추가
-- author_author_id를 참조하는 컬럼 author.author_id 컬럼과 같은 형태여야 한다.
...
-- book 테이블의 book_id 컬럼의 PRIMARY KEY 제약조건을 부여 
ALTER TABLE book
